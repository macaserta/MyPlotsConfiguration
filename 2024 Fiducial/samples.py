...
########## WWewk #########
'''
files = nanoGetSampleFiles(mcDirectory, 'WpWmJJ_EWK_noTop')
...
#addSampleWeight(samples, 'VgS', "DYGto2LG-1Jets_MLL-4to50_PTG-10to100_ext1", "(Gen_ZGstar_mass > 0)")
...
for _, sd in DataRun:
  for pd in DataSets:
    ...
    if tag.startswith('MuonEG'):
        ...
        files = nanoGetSampleFiles(fakeDirectoryMuon, tag)
    elif tag.startswith('EGamma'):
        files = nanoGetSampleFiles(fakeDirectoryEGamma, tag)
